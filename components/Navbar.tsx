...
export default function Navbar() {
  ...
  return (
    <Paper className="z-30 rounded-none sticky top-0 w-full p-0" shadow="lg">
      <header className="mx-auto max-w-[85rem] center justify-between h-20 p-5 lg:px-10">
            ...
            {NavbarLinks.map((r) => {
              ...
              const menuItems = r.links?.map((item) => (
                <Menu.Item key={item.link}>
                  <NextLink
                    className={`link text-base ${active === r.link ? 'text-blue-400' : ''}`}
                    href={item.link}
                  >
                    {item.label}
                  </NextLink>
              ...
              if (menuItems) {
                return (
                  <Menu key={r.label} trigger="hover" exitTransitionDuration={0}>
                    <Menu.Target>
              ...
              return (
                <div className={`link ${active === r.link ? 'text-blue-400' : ''}`} key={r.link}>
