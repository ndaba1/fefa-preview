...
const Footer = () => {
  ...
  const groups = lists.map((group) => {
    const routes = group.list.map((link, index) => (
      <Text key={index} className={classes.link} component={NextLink} href={link[1]}>
        {link[0]}
      </Text>
    ));
      ...
      <Container className={`${classes.inner} `}>
        <div className="flex flex-col space-y-3 items-center">
          <div className="max-w-[150px] ">
            <Image alt="fefa_academy" src={Logo} />
          </div>
